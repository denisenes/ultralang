...
let from_chan stream : char option = In_channel.input_char stream.source
...
let read_char stream : char =
  let res =
    ...
    | Some c ->
        c
    | None -> (
        match from_chan stream with
        | Some c ->
            if c = Token.newline then stream.line_num <- stream.line_num + 1;
            c
        | None ->
            raise End_of_file)
  in
...
let unread_char stream c : unit = Dynarray.add_last stream.buffer c
...
let rec skip_whitespaces stream =
  ...
  if Token.is_white c then skip_whitespaces stream else unread_char stream c
...
let unread_token stream (tok : string) : unit =
  if tok = "" then () else logMsg TOKENIZER ("[Stream] token unread" ^ tok);
  let tok' = tok ^ " " in
  (* because we don't want to join this token with the next *)
