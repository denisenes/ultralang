...
let read_from_file : ir_transformer = function
  | String s ->
      Channel (open_in s)
  | _ ->
      invalid_ir_kind "String" "Channel"
...
let write_image_to_file : ir_transformer = function
  | Image img ->
      ...
      Nothing
  | _ ->
      invalid_ir_kind "Image" "Nothing"

let printer : ir_transformer =
 fun data ->
  match data with
  | Module m ->
  ...
  | LLIR llir ->
      ...
      data
  | _ ->
      raise (InvalidIRKind "Printer input is invalid")
...
let stage ?(printable : bool = true) (name : string) : Stage.t =
  ...
  if printable then Stage.constr name action
...
let driver_main (args : string array) =
  ...
  let result =
    ...
    | "--compile"
    | "-c" ->
        driver_compile args.(2)
    | "--asm" ->
        driver_asm args.(2)
    | _ ->
        raise (ExecutionException "Execution mode is not defined")
  in
  match result with
  | Nothing ->
      print_endline "\nFinished";
      ()
  | _ ->
      raise (ExecutionException "Expected no result on the end of execution")
