...
module Stage = struct
  ...
  let constr name action = { name; action; is_printable = true }
  let constr_hidden name action = { name; action; is_printable = false }

  let execute stage input =
    if stage.is_printable then Printf.printf "\nStage: %s\n" stage.name;

    stage.action input
end

let stages_executor (data : stage_data) (stages : Stage.t array) : stage_data =
  let rec stages_executor' (stages : Stage.t array) (i : int)
      (data : stage_data) =
    if i >= Array.length stages then data (* End of execution *)
    else
