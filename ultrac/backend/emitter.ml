...
let emit_cunit (image : bytes) (unit : CUnit.t) : unit =
  let emit_value (width : Instruction.width) (addr : int) (v : int) =
    match width with
    | W8 ->
        Bytes.set_int8 image addr v
    | W16 ->
        Bytes.set_int16_be image addr v
  in
    ...
    ignore
      (List.fold_left
         (fun addr operand ->
           ...
           | Instruction.W8 hword ->
               emit_value W8 addr hword;
               addr + Utils.hword_size
           | Instruction.W16 word ->
               emit_value W16 addr word;
               addr + Utils.word_size)
         (start_addr + Instruction.opcode_size)
         instr.opnds)
  in
