...
let index_cunit (unit : CUnit.t) : unit =
  ...
  let advance (offset : int) : unit = current_addr := !current_addr + offset in
  ...
  CUnit.foreach_block unit (fun b ->
      ...
      | Reserved -> (
          ...
          | _ ->
              Shared.Errors.should_not_reach_here
                "Reserved block should contain only label")
      | Function ->
