module Node = struct
  ...
  let node kind ?(name = None) ?(args = []) ?(value = NoValue) ?(line = 0)
      ?(ntype = None) () : t =
    Log.logMsg Log.PARSER @@ Format.sprintf "New node %s" (show_node_kind kind);
    { kind; name; args; value; line; ntype }

  let identifier (name : string) ?(line = 0) () : t =
    node Ident ~name:(Some name) ~line ()

  let literal (v : value) ?(line = 0) () : t = node Literal ~value:v ~line ()
  ...
  let show (node : t) : string =
    let rec show0 n lvl =
      ...
      let header =
        Format.sprintf "├─%s %sval[%s] type[%s] ln[%s]\n" (show_node_kind kind)
          (if Option.is_some name then
             Format.sprintf "name[%s] " (Option.get name)
           else "")
...
let show_def_desc (dd : def_desc) : string =
  let header =
    Format.sprintf "\n\n%s name[%s] line[%s] type[%s]\n" (show_def_kind dd.kind)
      dd.name (string_of_int dd.line)
      (if Option.is_some dd.def_type then Option.get dd.def_type |> Type.show
       else "")
  in
  let sargs = Format.sprintf "args[%s]\n" @@ String.concat " " dd.args in
  let body0 =
    match dd.body_tree with
    | None ->
        ""
    | Some node ->
        Node.show node
