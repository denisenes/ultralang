(* bytes *)
(* TODO move to Shared.Isa.Platform *)
let word_size = 2
...
module UOption = struct
  let do_with_default (v : 't option) (action : 't -> 'a) (default : 'a) : 'a =
    if Option.is_some v then action @@ Option.get v else default

  let get_or_default (v : 't option) (default : 't) : 't =
    if Option.is_some v then Option.get v else default
end
