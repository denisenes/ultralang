...
let errorl (stream: Stream.t) (msg: string) = 
  let prefix = Printf.sprintf "[Line: %s] " (string_of_int stream.line_num) in
  error (prefix ^ msg)
...
and parse_exp (stream: Stream.t) : Node.t =
  ...
  if is_valid_ident tok then begin
    ...
    | '(' ->
      ...
      node Call ~args:(identifier tok () :: args) ~line:line ()
...
and parse_infix_exp (stream: Stream.t): Node.t =
  ...
  if is_infix_op token then
    ...
    let op = identifier token ~line:line () in
    node Call ~args:[op; exp1; exp2] ~line:line ()
  else begin
...
let parse_def (stream: Stream.t): def_desc =
  ...
  choice stream [
    ...
    ("const", fun stream ->
      ...
      let args = parse_def_args stream in
      let body = parse_infix_exp stream in
      {name=name; args=args; line=line; def_type=None; body_tree=Some body}
    );
  ]
